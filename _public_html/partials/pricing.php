<section class="pricing-section" id="pricing">
    <div class="section-header reveal">
        <span class="section-label">Pricing</span>
        <h2 class="section-title">Choose Your Access</h2>
        <p class="section-desc">Simple, transparent pricing. No hidden fees. Cancel anytime.</p>
    </div>
    <div class="pricing-grid">
        <div class="pricing-card card-community reveal">
            <div class="pricing-name"><i class="fas fa-users tier-icon"></i> Community</div>
            <div class="pricing-price">Free</div>
            <div class="pricing-subprice">Forever, no credit card</div>
            <ul class="pricing-features">
                <li>All games access</li>
                <li>Community forums</li>
                <li>Basic profile</li>
                <li>24/7 community</li>
            </ul>
            <a href="#" class="pricing-cta ripple-container" data-action="create-guest">Start Free</a>
        </div>
        <div class="pricing-card card-monthly reveal">
            <div class="pricing-name"><i class="fas fa-bolt tier-icon"></i> Premium Monthly</div>
            <div class="pricing-price"><span>/mo</span></div>
            <div class="pricing-subprice">Flexible, cancel anytime</div>
            <ul class="pricing-features">
                <li>All games</li>
                <li>Unlimited AI chat</li>
                <li>Custom profile themes</li>
                <li>Priority support</li>
            </ul>
            <a href="shop.php?plan=monthly" class="pricing-cta ripple-container">Get Monthly</a>
        </div>
        <div class="pricing-card card-yearly reveal">
            <i class="fas fa-crown yearly-crown"></i>
            <div class="yearly-badges">
                <span class="yearly-badge popular">? Most Popular</span>
                <span class="yearly-badge savings">Save 11%</span>
            </div>
            <div class="pricing-name">Premium Yearly</div>
            <div class="yearly-price-wrap">
                <span class="yearly-price-old"></span>
                <div class="pricing-price" id="yearlyPrice"><span>/yr</span></div>
            </div>
            <ul class="pricing-features">
                <li>Everything in Monthly</li>
                <li>2 months free vs. monthly</li>
                <li>Annual member badge</li>
                <li>Priority support</li>
            </ul>
            <a href="shop.php?plan=yearly" class="pricing-cta ripple-container">Get Yearly</a>
        </div>
    </div>
</section>
